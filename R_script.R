#1 Installing and loading all the necessary packages
...
#2 Setting the directory for R to import the files

getwd()

#3 Importing necessary .csv files into R
...
#4 There are variations in the data types of start_station_id and end_station_id, we need to fix this
...
#5 Converting the data type of started_at/ended_at from character to datetime format (Jan_2021, Feb_2021 & Mar_2021)
...
#6 Merging all the tables into a single data set
...
#7 Eliminating unwanted columns/data from the data set

final_data <- final_data %>% select(-c(start_lat, end_lat, start_lng, end_lng))

#8 Analyzing surface level data from the filtered/cleaned data set
...
#9 Segregating Date, day, month and year into separate columns
...
#10 Calculating the ride time for each of the rides

final_data$ride_length <- difftime(final_data$ended_at, final_data$started_at)

#11 Eliminating unwanted data from the data set to get meaningful insights

is.factor(final_data$ride_length) #To ensure if ride_length is a factor or not. TRUE = yes and FALSE = no 
...
final_data_v2 <- final_data[!(final_data$start_station_name == "" | final_data$ride_length<0),] #Created a new version by removing all the unwanted data from the data set

#12 Analyzing the v2.0 of the data set with respect to the ride_length

summary(final_data_v2$ride_length)

#13 Sorting data based on rider type details (casual/member) 
...
#14 Calculating average time by day of the week with respect to rider type
...
#15 Further analyzing the rider data w.r.t. ride length and day of the week
...
#16 Creating visualizations using ggplot
#Viz.1
...
#17 Saving a new copy of file for further analysis
